...
function iptEzPDF($paper='a4',$orientation='portrait'){
...
function ezStream($options=''){
	//$this->ezPRVTcleanUp();
  	//$this->stream($options);
	$dt=date("YmdHis").".pdf";
	$f=fopen("/home/oms/www/rhinhost.com/pdfwrap/".$dt,"wb");
	fputs($f,$this->ezOutput());
	fclose($f);
	echo "<script>\n";
	echo "window.location.href='http://www.rhinhost.com/pdfwrap/".$dt."'\n";
	echo "parent.resizeTo(window.screen.availWidth,window.screen.availHeight)\n";
	echo "parent.moveTo(0,0)\n";
	echo "</script>\n";
  
}

function ezStream2($options=''){


        $this->ezPRVTcleanUp();
	 $this->stream($options);
	//	        $dt=date("YmdHis").".pdf";
//			        $f=fopen("/home/wz/sofadeco.com/pdfwrap/".$dt,"wb");
//
//      $dt=date("YmdHis").".pdf";
  //            $f=fopen("/home/wz/sofadeco.com/pdfwrap/".$dt,"wb");
//	              fputs($f,$this->ezOutput());
...
//	print      file_get_contents("/home/wz/sofadeco.com/pdfwrap/".$dt);
			...
			/*		        fclose($f);
						        echo "<script>\n";
							        echo "window.location.href='http://www.sofadeco.com/pdfwrap/".$dt."'\n";
								        echo "parent.resizeTo(window.screen.availWidth,window.screen.availHeight)\n";
									        echo "parent.moveTo(0,0)\n";
										        echo "</script>\n";
...
function ezSaveToFile($filename){
	//$this->ezPRVTcleanUp();
  	//$this->stream($options);
	$dt=date("YmdHis").".pdf";
	$f=fopen($filename,"wb");
...
function ezTable(&$data,$cols='',$title='',$options=''){
  ...
  // 'showHeadings' => 0 or 1
  // 'shaded'=> 0,1,2,3 default is 1 (1->alternates are shaded, 0->no shading, 2-> both shaded, second uses shadeCol2)
...
function ezImage($image,$pad = 5,$width = 0,$resize = 'full',$just = 'center',$border = ''){
	...
	if (stristr($image,'://'))//copy to temp file
	{
		...
		while(!feof($fp))
   		...
   		fclose($fp);
		$image = tempnam ("/home/seahorse/shared/temp", "php-pdf");
		$fp2 = @fopen($image,"w");
   		fwrite($fp2,$cont);
  		fclose($fp2);
		$temp = true;
