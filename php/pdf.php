...
*/
function iptPDF ($pageSize=array(0,0,612,792)){
...
function addImage(&$img,$x,$y,$w=0,$h=0,$quality=75){
  ...
  $tmpDir='/home/media6/www/admin.media6.ca/tmp';
...
function wpdf_data($touttexte, $colonne, $ligne, $largeur, $hauteur, $mode, $extensible, $type, $nexthauteur, $souligne=0, $libelle=""){
	$touttexte=toweb($touttexte);
	$libelle=toweb($libelle);
	$imprime=1;
	switch ($type){
	case 4: //money
		$touttexte=str_replace(",",".",$touttexte);
		$touttexte=number_format($touttexte,2,',',' ');
		//if ($touttexte==""){$touttexte="0,00";}
		//if ($touttexte=="0"){$touttexte="0,00";}
		//$touttexte=str_replace(".",",",$touttexte);
		//if (strpos($touttexte,",")<1){$touttexte=$touttexte.",00";}
		//if (substr($touttexte,strlen($touttexte)-5,1)==","){$touttexte=substr($touttexte,0,strlen($touttexte)-2);}
		if ($touttexte=="0,00"){$imprime=0;}
		$mode=right;
		break;
	case 5: //date
		if ($touttexte!=""){$touttexte=substr($touttexte,6,2).'/'.substr($touttexte,4,2).'/'.substr($touttexte,0,4);}
		break;
	default:
	}	
	if ($libelle!=""){
		$touttexte=$libelle.": ".$touttexte;
	}
	if (($imprime==1)&&($touttexte!='')){
		$etape=0;
		if ($extensible==2){$this->setcolor(1,1,1);}
		while ($etape<=1){
			$pos2=0;
			$pos1=-2;
			$pos2=strpos($touttexte,"\\n");
			$flfin=0;
			while ($flfin==0){
				if ($pos2>$pos1){
					$texte=substr($touttexte,$pos1+2,$pos2-$pos1-2);
				}
				else{
					$flfin=1;
					$texte=substr($touttexte,$pos1+2);
				}
				$hauteurtotal+=$hauteur;
				
				if ($extensible!=0){
					//while ($texte!=""){
						$texte=$this->addTextWrap(($colonne+2),(792-$hauteurtotal-$ligne),($largeur-4),$hauteur,$texte,$mode,0,0);
						$hauteurtotal+=$hauteur;
					//}
					$hauteurtotal-=$hauteur;
				}
				else{
					$texte=$this->addTextWrap(($colonne+2),(792-$ligne-$hauteur),($largeur-4),$hauteur,$texte,$mode,0,0);
					$flfin=1;
				}
				if (($etape==1)||($extensible!=2)){
					if ($souligne==1){
						$hauteurtotal+=2;
						$this->line($colonne+2,(792-$ligne-$hauteurtotal),($colonne+$largeur-2),(792-$ligne-$hauteurtotal));
					}	
					if ($souligne==2){
						$this->line($colonne+2,(792-$ligne),($colonne+$largeur-2),(792-$ligne));
					}	
					if ($souligne==3){
						$hauteurtotal+=2;
                                                $this->setStrokeColor(192,192,192);
						$this->line($colonne+2,(792-$ligne-$hauteurtotal),($colonne+$largeur-2),(792-$ligne-$hauteurtotal));
                                                $this->setStrokeColor(0,0,0);
					}	

				}
				if ($hauteurtotal>$nexthauteur){$nexthauteur=$hauteurtotal;}
			...
			if ($extensible==2){
				$this->setcolor(0,0,0);
				$ligne=$ligne-$hauteurtotal+$hauteur;
				$hauteurtotal=0;
			}
			else{
				$etape=9;
...
function wpdf_libelle($colonne,$ligne,$largeur,$hauteur,$taillepolice,$libelle,$data,$mode,$type) {
	$this->setcolor(.9, .9, .9); 
	$this->filledrectangle($colonne,792-$ligne-$hauteur,$largeur,$hauteur);
	$this->rectangle($colonne,792-$ligne-$hauteur,$largeur,$hauteur);
	$this->setcolor(0,0,0); 
	$mode=center; 
	if ($type==4){$mode=right;}
	$this->addTextWrap($colonne+2,792-$ligne-$hauteur+(($hauteur-$taillepolice)/2),$largeur-4,$taillepolice,$libelle,$mode); 
}

/*******************************************************************************************/

function wpdf_libelle_data($colonne,$ligne,$largeur,$hauteur,$taillepolice,$libelle,$touttexte,$mode,$type) {
	$touttexte=toweb($touttexte);
	$this->setcolor(.9, .9, .9); 
	$this->filledrectangle($colonne,792-$ligne-15,$largeur,15);
	$this->rectangle($colonne,792-$ligne-15,$largeur,15);
	$this->setcolor(0,0,0); 
	$this->rectangle($colonne,792-$ligne-15-$hauteur,$largeur,$hauteur);
	if ($type==4){$mode=right;}
	//$this->addTextWrap($colonne+2,792-$ligne-$hauteur+((15-$taillepolice)/2),$largeur-4,$taillepolice,$libelle,$mode); 
	$this->addTextWrap($colonne+2,792-$ligne-15+((15-$taillepolice)/2),$largeur-4,$taillepolice,$libelle,$mode); 
	switch ($type){
	case 4: //money
		if ($touttexte==""){$touttexte="0,00";}
		if ($touttexte=="0"){$touttexte="0,00";}
	...
	if ($touttexte!=''){
		...
		while ($flfin==0){
			...
			$hauteurtotal+=$hauteur;
			$this->addTextWrap($colonne+2,792-$ligne-$hauteurtotal+(($hauteur-$taillepolice)/2)-15,$largeur-4,$taillepolice,$texte,$mode); 
			$pos1=$pos2;
			$pos2=strpos($touttexte,"\\n",$pos1+1);
		}
...
function wpdf_bloc_data($colonne,$ligne,$largeur,$hauteur,$taillepolice,$libelle,$touttexte,$mode,$type) {
	$touttexte=toweb($touttexte);
	$this->setcolor(.9, .9, .9); 
	$this->filledrectangle($colonne,792-$ligne-15,$largeur,15);
	$this->rectangle($colonne,792-$ligne-15,$largeur,15);
	...
	if ($touttexte!=''){
		...
		while ($flfin==0){
			...
			$hauteurtotal+=$taillepolice;
			$this->addTextWrap($colonne+2,792-$ligne-$hauteurtotal-15,$largeur-4,$taillepolice,$texte,$mode); 
			$pos1=$pos2;
			$pos2=strpos($touttexte,"\\n",$pos1+1);
		}
